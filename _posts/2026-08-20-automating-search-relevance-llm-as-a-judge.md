...
However, the evaluation of search relevance remains a complex, ongoing challenge. Online signals such as A/B tests and
engagement metrics tell us whether a change moves business KPIs, but they don’t directly measure how relevant individual
results are to a given query. For years, the industry standard relied on a
...
<figure>
<img alt="Canonical example of search relevance annotations mapping user intent to retrieved products" src="/assets/img/articles/2026-08-20-automating-search-relevance-llm-as-a-judge/annotation-canonical-example.png" />
<figcaption>
...
product. (AI generated image)
</figcaption>
</figure>

<figure>
<img alt="Challenging example of labeling products retrieved for an ambiguous user query" src="/assets/img/articles/2026-08-20-automating-search-relevance-llm-as-a-judge/annotation-ambiguous-example.png" />
<figcaption>
More challenging example: how to label products retrieved for an ambiguous user query? Do they all deserve the <em>Exact
product</em> label? (AI generated image)
</figcaption>
</figure>
...
<figure>
<img alt="Chart showing Cohen's kappa scores rising between the 2023 and 2024 annotation tasks" src="/assets/img/articles/2026-08-20-automating-search-relevance-llm-as-a-judge/cohen-kappa-evolution.png" />
<figcaption>
Evolution of Cohen’s kappa scores across two iterations of the search relevance annotation task. The shift in kappa highlights the positive impact of structured feedback loops and granular
annotation guidelines on team alignment (conceptual difficulty of the dataset — measured by the ratio of canonical cases to
complex edge examples — remained comparable between both tasks). (AI generated image)
</figcaption>
</figure>
...
benefit when building on a solid expert-written prompt rather than generating a strong prompt on their own.

We started with a deliberately simple, text-only approach — the product name is the only input we feed to the model, as it
already contains the necessary product information. We also tried adding explicit department and product category information to
the prompt, but dropped it because it did not improve output quality in our offline tests — these details can typically be
inferred from the product name itself.

### The paradox of examples

Initially, we also experimented with few-shot prompting to guide the model. Surprisingly, our offline experiments (ablations — removing one prompt component at a time) revealed
that removing few-shot examples actually improved model performance. The prompt without few-shot examples yielded higher
accuracy and agreement scores compared to the few-shot version (for more on metrics used to rate LLM performance see
[the section below](#model-selection-and-per-class-metrics)). This suggests that for complex relevance tasks, providing too many examples can introduce noise or bias the model away
from the core logic.
...
focusing on model accuracy and agreement scores with human ground truth values. Our analyses revealed that
...
system evaluates how effectively the retrieved products fulfill the user intent encoded in the search query, using
LLM-as-a-judge evaluation metrics — such as relevance label-based [NDCG](https://en.wikipedia.org/wiki/Discounted_cumulative_gain) — to score search results.
...
on-demand search results annotation and batched model requests for daily search quality verification.

While these two workflows serve different business and product goals, they share the exact same underlying engineering
backbone. Whether we are processing a single listing (the ranked page of products returned for a query) in real-time or running thousands of search queries through a nightly cron
...
<figure>
<img alt="High-level data flow of the Relevance Assessment Tool pipeline" src="/assets/img/articles/2026-08-20-automating-search-relevance-llm-as-a-judge/colors_corrected_pipeline.png" />
<figcaption>
High-level data flow of the Relevance Assessment Tool pipeline.
</figcaption>
</figure>

The pipeline highlights the following key processes.
...
**A note on batching.** Notice that we provide the model with the entire product page (Products A through Z) in a single
API request, rather than firing individual requests for each product. Packing the full context into one session
reduces network round-trips, optimizes token usage, and cuts costs.
...
and quality by preventing context dilution. This matters most for local models, which struggle to return correct answers
when a single request contains too many products to annotate. It’s a classic balance between performance and accuracy.
...
To operationalize this pipeline, a single shared codebase serves two different operational needs.
...
running large-scale evaluations on cloud-based LLM APIs is expensive, so moving to local models was the natural next
step.
...
variants to make sure we wouldn’t compromise the quality established by our cloud baseline (Gemini 3.1 Flash Lite, shown in the
last column of the table below). Across all four languages, one model stood out — Gemma 4 26B (AWQ 4-bit, no-thinking) — which
for the Polish language exceeded the quality of our Gemini 3.1 Flash Lite baseline. On CZ, SK and HU datasets the cloud baseline
retained an edge.

The table below reports the following metrics:

* **κ-quad** (Cohen’s Kappa quadratic) — agreement with human labels on our ordinal scale, penalizing large rating
  disagreements much more than near misses (−1 to 1, higher is better). Unlike standard (unweighted) Cohen’s kappa, which
  treats every disagreement equally, the [quadratic weighted variant](https://www.sciencedirect.com/science/article/abs/pii/S1572312711000773)
  weights each disagreement by the squared distance between ratings, so mislabeling an *Exact product* as *Irrelevant* is
  penalized far more heavily than confusing two adjacent classes.
* **Accuracy** — share of exactly correct labels.
* **F1** — balanced score that rewards getting all classes right rather than over-predicting the majority class.
* **[MAE](https://en.wikipedia.org/wiki/Mean_absolute_error)** — average distance between the model’s rating and the true
  rating (0–4, lower is better).

| Language (dataset) | Best local model | κ-quad | Accuracy | F1 | MAE | Cloud baseline κ-quad (Gemini 3.1 Flash Lite) |
|---|---|---:|---:|---:|---:|---:|
...
**Table 2: comparison of the best local model per language with the baseline cloud-based model** (Cohen’s κ quadratic is the
primary metric).

Higher κ means better agreement with human ground truth. `cyankiwi-gemma` (Gemma 4 26B AWQ 4-bit, no-thinking) is the best
local model for Polish. Because the cloud baseline continues to outperform local options for the remaining languages (CZ, SK,
HU), Gemini 3.1 Flash Lite is still suggested as the primary reliable choice.
...
* **Product list size**: for Gemini models, the prompt includes the whole first page of Allegro’s products to reduce
  ...
  prompt. The whole-product-page and 10-products-per-prompt variants proved ineffective as most of the model requests failed to
  parse the response (corrupted JSON, incorrect number of products). We therefore opted for single-item requests to bypass
  these parsing errors and ensure output reliability.
* **Throughput**: despite being our largest local model, Gemma 4 26B (no-thinking) was also the fastest in practice, sustaining
  around 16 query-product pairs per second — roughly 2.5x the throughput of the smaller Gemma 4 12B variant (on the same
  hardware) — which made wide-scale, continuous evaluation feasible without sacrificing quality.
* **Multilingual performance**: our evaluation spanned four languages (PL, CZ, SK, HU). Polish served as our primary benchmark
  ...
  outperformed our best local model on Slovak and Hungarian, making it the more reliable choice for those
  languages.
...
cleanly ingested into our pipelines. Models that couldn’t meet this reliability bar (e.g. Mellum 2 12B AWQ — parse errors, or
PLLuM 8B — class-imbalance exploitation) were ruled out regardless of high accuracy (see the table below).
...
**Table 3: model comparison on the primary benchmark (PL).** **Accuracy** is the raw share of correct labels and can be
...
The result: by deploying the best-performing local model, we matched the quality of our cloud baseline while
cutting inference costs by roughly 60%. This lets us run continuous, wide-scale relevance assessment — without the linear per-call cost scaling that cloud
APIs demand.
...
The present article is the result of collaboration of the Narwal (Search relevance) and Data Annotation & QA Services (DAQAS) teams.
We extend our sincere thanks to the Pytia (Search Analytics), LTR (Learning to Rank) and Phi (Search user guidance) teams for
their invaluable contributions to accuracy metrics, RAT development and UX design.
