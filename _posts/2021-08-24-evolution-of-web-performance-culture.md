...
better performance are long-term investment, and how to streamline the development process during the design or code writing process.

Web performance is a very difficult and complex subject. It involves working at the intersection of content management,
...
but the pace was not sufficient. We lacked business support, which would see profit in all of this.

## Stage Two — Make it clear that performance is important

We went a long way making the entire organization realize that the gains from web performance are long term and important overall.

Everyone subconsciously knew that a faster site affects user experience. The earlier users are able to see a page and use it,
...
Although being a proof of concept, this experiment turned out to be an enabler for our next steps.
It helped us better understand what we are aiming for.

This experiment has opened the way for us to make more optimizations, it also provided us with solid argument
...
Thanks to that we mitigate the risk that bad patterns will be repeated again.
...
In our team we assume that everything that is possible should be automated. We want our tools to support
...
* asset sizes on the page are appropriate,
* they are cached for long enough,
...
Two comments are generated while building the component. The first presents a comparison of assets size
with the target branchand the estimated cost of the change.
...
In the second one, Lighthouse reports are compared in order to catch performance metrics’ regressions at this early stage.
...
Lighthouse report is generated for one or more presets every time the component is built.
...
We get similar notifications when assets size change by at least 5% compared to target branch.
We want to make sure that, for example, treeshaking hasn't broken down or some other change affecting the size has occurred.
...
Keep in mind that working on web performance is a continuous work. Every now and then new solutions appear on
the market which may have a positive impact on the loading speed. Therefore, together with our team,
we run a series of tests to see if it makes sense to adopt the given solution.

### Appreciation culture

We believe that the carrot is better than the stick, so we try to praise the achievements of other teams
...
We are no longer fighting alone as a Webperf team, but as an entire organization.