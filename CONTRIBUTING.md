...
We are in the process of automating some parts of the publication process using Github Copilot. See [.github/copilot-instructions.md](.github/copilot-instructions.md)
for details. In particular, you can ask Copilot in agent mode to:
- check your blog post (it should take into account the guidelines from this document)
- suggest social media posts based on your blog post
- prepare your blog post for publication (renaming files, updating image links, etc.)

As always, this is a tool, not a replacement for your own thinking or personal style.
...
0. With at least two approvals from blog editors, the PR is (almost) ready for publication.
...
0. After consultation with post author, one of the editors updates the publication date.
0. PR is merged by one of the editors. This triggers post publication
   process via [exiting GH actions](https://github.com/allegro-internal/blog/tree/main/.github/workflows).
...
* If this is your first post, also add following information:
    ...
    * A file called `assets/img/authors/firstname.lastname.jpg` which should be your picture in square format (face only).
...
Choose `agile` if you want to publish on Agile Blog.
...
## Headers
If you use only one level of headers, use ## (h2).
...
- ruby (preferably, use `rbenv`)
- npm (preferably, use `nvm`)

The files `.ruby-version` and `.node-version` in the repo store versions that `rbenv` and `nvm`, respectively, read.
They should be kept up to date with the [versions used by Github Pages](https://pages.github.com/versions.json), and the node version
should be the same as used by `setup-node` action in our Github workflows (`.github/workflows/*.yml`).
Use these versions for local testing even if they are outdated, since that's the closest you can get to the production environment.
...
Github workflows are used to publish the blog post, which involves pushing code from the internal repository (`allegro-internal` organization) to the public one
(`allegro` organziation).This operation requires certain permissions. For those, we rely on Github apps whose credentials are stored in project secrets in the
`allegro-internal/blog` project. You can find the names by reading workflow definitions. The blog editorial team also have access to these secrets via the
